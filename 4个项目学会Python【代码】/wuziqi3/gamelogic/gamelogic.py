...
class GameLogic:
    ...
    def isCPUWin(self):
        ...
        for i in range(1, 5):
            if self.cpu_row+ i > 0 and self.cpu_col + i < 9 and self.board.isWhite(self.cpu_row + i, self.cpu_col + i):
                ...
                break
        for i in range(-1,-5,-1):
            if self.cpu_row + i < 9 and self.cpu_col + i > 0 and self.board.isWhite(self.cpu_row + i, self.cpu_col + i):
                count += 1
            else:
                break
        if count >= 5:
    ...
    def playerRound(self):
        ...
        self.player_row = int(player_input[0]) - 1
        self.player_col = int(player_input[1]) - 1
    ...
    def cpuRound(self):
        ...
        youXiaCol = 1
        continueCount = 1
        while self.player_row + zuoShangRow > 0 and self.player_col + zuoShangCol > 0 and self.board.isBlack(
                self.player_row + zuoShangRow, self.player_col + zuoShangCol):
            continueCount += 1
            zuoShangRow -= 1
            zuoShangCol -= 1
        while self.player_row + youXiaRow < 9 and self.player_col + youXiaCol < 9 and self.board.isBlack(self.player_row + youXiaRow, self.player_col + youXiaCol):
            continueCount += 1
            youXiaRow += 1
            youXiaCol += 1
        if continueCount >= maxContinueCount:
        ...
        youShangCol = 1
        continueCount = 1
        while self.player_row+ zuoXiaRow < 9 and self.player_col + zuoXiaCol > 0 and self.board.isBlack(
            self.player_row + zuoXiaRow, self.player_col + zuoXiaCol):
            continueCount += 1
            zuoXiaRow += 1
            zuoXiaCol -= 1
        while self.player_row + youShangRow > 0 and self.player_col + youShangCol < 9 and self.board.isBlack(
                self.player_row + youShangRow, self.player_col + youShangCol):
            continueCount += 1
            youShangRow -= 1
            youShangCol += 1
